...
func InjectLoggerInterceptor(rootLogger *zerolog.Logger) grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req interface{}, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (resp interface{}, err error) {
		...
		traceID, spanID, sampled := traceContextFromHeader(values[0])
		if traceID == "" {
			return handler(ctx, req)
		}
		...
		l.UpdateContext(func(c zerolog.Context) zerolog.Context {
			return c.Str("logging.googleapis.com/trace", trace).Str("logging.googleapis.com/spanId", spanID).Bool("logging.googleapis.com/trace_sampled", sampled)
		})
